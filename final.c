...
int main(void)
{
    ...
    for(int i=0;i<epoch;i++){
        ...
        for(int j=0;j<train_count/batch_size;j++){
            ...
            scale(784*50,-learning_rate/((float)batch_size),dEdA1_ave);
            scale(50,-learning_rate/((float)batch_size),dEdb1_ave);
            scale(50*100,-learning_rate/((float)batch_size),dEdA2_ave);
            scale(100,-learning_rate/((float)batch_size),dEdb2_ave);
            scale(100*10,-learning_rate/((float)batch_size),dEdA3_ave);
            scale(10,-learning_rate/((float)batch_size),dEdb3_ave);
